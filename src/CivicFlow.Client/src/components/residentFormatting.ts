...
export function residentServiceMessage(item: ResidentStatus) {
  ...
  if (item.slaState === 'AtRisk') return 'Service target approaching'
...
export function notificationGroupLabel(value: string, now = new Date()) {
  ...
  if (days === 0) return 'Today'; if (days === 1) return 'Yesterday'; return target.toLocaleDateString([], { dateStyle: 'long' })
}
