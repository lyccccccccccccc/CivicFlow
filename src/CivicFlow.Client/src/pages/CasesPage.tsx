...
export function CasesPage() {
  ...
  return <Stack spacing={3}>
    <PageHeader title={heading} description={resident ? 'Track every update from submission to closure.' : `${officer ? 'Prioritise and progress work assigned to you.' : 'Review, assign and manage work across the service team.'}${result ? ` ${result.totalCount} result${result.totalCount === 1 ? '' : 's'}.` : ''}`} actions={resident && <Button component={Link} to="/requests/new" variant="contained" startIcon={<AddRoundedIcon />}>New request</Button>} />
    {!resident && quickViews.length > 0 && <Stack direction="row" spacing={1} useFlexGap role="group" aria-label="Case quick filters" sx={{ flexWrap: 'wrap' }}>{officer && <Button aria-pressed={!params.get('quickView')} variant={!params.get('quickView') ? 'contained' : 'outlined'} onClick={() => update('quickView', '')}>My cases</Button>}{quickViews.map(([label, value]) => <Button key={value} aria-pressed={params.get('quickView') === value} variant={params.get('quickView') === value ? 'contained' : 'outlined'} onClick={() => update('quickView', params.get('quickView') === value ? '' : value)}>{label}</Button>)}</Stack>}
    <Paper sx={{ p: 2 }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={2} useFlexGap sx={{ flexWrap: 'wrap' }}>
      ...
      {canAssign && <TextField select size="small" label="Officer" value={params.get('officerId') ?? ''} onChange={e => update('officerId', e.target.value)} sx={{ minWidth: 240 }}><MenuItem value="">All</MenuItem>{officers.map(x => <MenuItem key={x.id} value={x.id} sx={{ whiteSpace: 'normal', overflowWrap: 'anywhere' }}>{officerLabel(x)}</MenuItem>)}</TextField>}
    ...
    <ActiveFilterSummary search={params.get('search') ?? undefined} status={params.get('status') ?? params.get('quickView') ?? undefined} onClear={clear} />
    ...
    </TableRow>)}{result?.items.length === 0 && <TableRow><TableCell colSpan={resident ? 6 : 9}><EmptyState title={resident ? 'No requests found' : 'No matching cases'} description={resident ? 'Submit a service request or change the current filters.' : 'Try changing or clearing the current filters.'} action={resident && <Button component={Link} to="/requests/new" variant="contained">Submit request</Button>} /></TableCell></TableRow>}</TableBody></Table></Paper>} mobile={<Stack spacing={4}>{result?.items.map(item => resident ? <ResidentRequestCard key={item.id} item={item} /> : <StaffCaseCard key={item.id} item={item} showOfficer={!officer} />)}{result?.items.length === 0 && <Paper><EmptyState title={resident ? 'No requests found' : 'No matching cases'} description={resident ? 'Submit a service request or change the current filters.' : 'Try changing or clearing the current filters.'} action={resident && <Button component={Link} to="/requests/new" variant="contained">Submit request</Button>} /></Paper>}</Stack>} />
      <Paper sx={{ mt: 3 }}>
...
const officerLabel = (officer: Officer) => `${officer.firstName} ${officer.lastName}${officer.email ? ` — ${officer.email}` : ''}`
