...
export function NewRequestPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); const validation = validate(form); setErrors(validation); if (Object.keys(validation).length) return; setBusy(true); setError(''); try { const created = await api<{ id: string }>('/cases', { method: 'POST', body: JSON.stringify({ ...form, title: form.title.trim(), description: form.description.trim(), address: form.address.trim() }) }); navigate(`/cases/${created.id}`) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to submit request') } finally { setBusy(false) } }
